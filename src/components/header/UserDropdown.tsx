...
export default function UserDropdown() {
  ...
  // The signed-in user's real name + photo (kept in sync with the profile page).
  ...
  const displayName =
    fullName?.trim() || session?.user?.email?.split("@")[0] || "Guest";
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="flex items-center justify-center mr-3 overflow-hidden text-sm font-semibold text-white rounded-full h-11 w-11 bg-brand-500">
          {avatarUrl ? (
            <img
              src={avatarUrl}
              alt={displayName}
